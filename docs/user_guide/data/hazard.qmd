...
In each case, some requirements must be met. The hazard files of any map should be made available as a **raster file**, see the acceptable file types at the bottom of this page. It is recommended to place the hazard files in the 'hazard' folder, see the [folder structure](data.qmd#folder-structure). The file name is free to be chosen by the user and must simply be refered to in the [settings.toml](settings.qmd) configuration file.
