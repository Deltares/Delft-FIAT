...
In each case some requirements must be met. For both scenarios the hazard files of any map should be made available as a **raster or vector file**  (see acceptable file types at the bottom of this page) and must be located in the 'hazard' folder. The file name is free to choose by the user and must simply be refered to in the *settings.toml* configuration file.
