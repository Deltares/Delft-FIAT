...
title: "Settings file"
---
The user must set the standard model settings in the `settings.toml` configuration file. Beyond standard settings FIAT offers the freedom to provide additional input to create a more detailed model (@tbl-toml) . More advanced options are described in the [Advanced options](../advanced.qmd) page.
...
[**exposure.geom**]
`csv`: This will create an exposure CSV file within the [exposure folder](folder_structure.qmd) that contains the [required information](exposure.qmd) per asset.
...
[**vulnerability**]
`file`: This will create an vulnerability curves CSV file within the [vulnerability folder](folder_structure.qmd) that contains the [damage curves](vulnerability.qmd).
